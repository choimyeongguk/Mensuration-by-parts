...
for n in range(0, 101):  # 함수를 0~100개 까지 나눔
 for i in range(0, n):  # n개 만큼 박스 생성
  ...
  plt.gca().add_patch(ptch.Rectangle((spot1+(spot2-spot1)/n*i, 0),
  (spot2-spot1)/n, fc(spot1+(spot2-spot1)/n*(i+1)), color=num))  # 박스 생성
  S = S + abs(1/n*fc(spot1+(spot2-spot1)/n*(i+1)))  # 넓이 계산
